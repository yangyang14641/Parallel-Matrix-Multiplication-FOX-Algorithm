...
main(int argc, char* argv[]) {
    ...
    Read_matrix("Enter A", local_A, &grid, n);           // Read local matrices A from process 0 by using stdin, and send them to each process (Procedure)
    ...
    Read_matrix("Enter B", local_B, &grid, n);           // Read local matrix B from process 0 by using stdin, and send them to each process (Procedure)
    Print_matrix("We read B =", local_B, &grid, n);      // Print local matrix B from process 0 by using stdout, and send them to each process (Procedure)
    ...
    Print_local_matrices("Split of local matrix B", 
                         local_B, &grid);                // Print matrix B split in processess
...
void Fox(
    ...
    // compute local matrix's order
    n_bar = n/grid->q;
    // set local matrix local_C's value to zero
    Set_to_zero(local_C);                            
    ...
    tempe_A = Local_matrix_allocate(n_bar);
...
void Build_matrix_type(
    ...
    MPI_Type_contiguous(Order(local_A)*Order(local_A), 
        MPI_FLOAT, &temp_mpi_t);                         // Creates a contiguous datatype
    /*
    Synopsis
...
void Local_matrix_multiply(
    ...
    for (i = 0; i < Order(local_A); i++)                           // Threads acceleration upgrade, local sum and global sum
        for (j = 0; j < Order(local_A); j++)
            for (k = 0; k < Order(local_B); k++)
                Entry(local_C,i,j) = Entry(local_C,i,j) 
                    + Entry(local_A,i,k)*Entry(local_B,k,j);       // non-continue memory access, should be upgrade A(i,k)*B'(j,k) is more proper
