/* fox_floats_timer_caching.c  -- uses Fox's algorithm to multiply two square matrices
 *
 * Input:
...
/* Compiler command: 
* mpiicc -O3 fox_floats_timer_caching.c -o fox_floats_timer_caching 
* 
* Run command:
* mpirun -n -4 ./fox_floats_timer_caching
*/
        ...
        for (j = 0; j < Order(local_A); j++)
            for (k = 0; k < Order(local_B); k++)
               ...
               /* Entry(local_C,i,j) = Entry(local_C,i,j) 
                    + Entry(local_A,i,k)*Entry(local_B,k,j);        // non-continuous memory access, should be upgrade A(i,k)*B'(j,k) is more proper 
               */
